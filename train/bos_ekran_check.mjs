...
//   6. the version bump: no `?v=74` may survive anywhere under web/.
...
const zaman = await import('../web/js/zaman.js?v=75');
const ui = await import('../web/js/ui.js?v=75');
const api = await import('../web/js/api.js?v=75');
...
await blok('bump olcumu', async () => {
  ...
  let kod = 0;
  try {
    cikti = execFileSync('git', ['grep', '-n', '-F', '?v=74', '--', 'web/'],
      { cwd: REPO, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
  } catch (e) { kod = e.status; cikti = String(e.stdout || ''); }
  ok('web/ altinda hicbir ?v=74 kalmadi', kod === 1 && !cikti.trim(), cikti.trim());

  let v75 = '';
  try {
    v75 = execFileSync('git', ['grep', '-c', '-F', '?v=75', '--', 'web/'],
      { cwd: REPO, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
  } catch (e) { v75 = String(e.stdout || ''); }
  const toplam = v75.split('\n').filter(Boolean)
    .reduce((a, l) => a + Number(l.split(':').pop() || 0), 0);
  console.log(`     ?v=75 gecen satir sayisi: ${toplam}`);
  ok('yerine ?v=75 kondu', toplam >= 30, String(toplam));

  const idx = readFileSync(join(REPO, 'web/index.html'), 'utf8');
  const imza = (idx.match(/class="imza"[^]*?<\/span>\s*$/m) || [])[0] || idx.match(/· v\d+ ·/)?.[0] || '';
  ok('index.html altbilgi surum etiketi v75', /· v75 ·/.test(idx), imza);
  ok('altbilgide eski v72/v73/v74 etiketi kalmadi', !/· v7[1234] ·/.test(idx));
});
